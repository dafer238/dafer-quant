// ./pmm-core/src/modules/owners.rs
...
impl Owner {
    ...
    pub async fn create_owner(db: &Database, owner: &Owner) -> Result<(), PMMError> {
        ...
        match res {
            ...
            Err(sqlx::Error::Database(e)) if e.message().contains("UNIQUE constraint failed") => {
                Err(PMMError::UsernameOrEmailTaken)
            }
            Err(e) => Err(PMMError::Db(e)),
        }
    }
