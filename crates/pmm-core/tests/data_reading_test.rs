// ./pmm-core/tests/data_reading_test.rs
...
fn test_csv_movements_loading() {
    ...
    let root_wd = pmm_utils::general::get_root_wd();

    // Build path for first CSV
    let movements_file_path = root_wd.join("data/input/movements.csv");

    let movements = CsvReadOptions::default()
