// ./jsl_core/tests/owners_db_test.rs

use jsl_core::database::sqlite_db::Database;
use jsl_core::modules::owners::{Owner, UserGroup};
...
async fn init_db_for_tests() -> Database {
    ...
    dotenv().ok();
    let db_path = jsl_utils::general::get_root_wd().join("data/database/db.sqlite");
